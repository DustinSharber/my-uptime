...
def download_prebuilt_agent():
    ...
    if not agent_path or not agent_path.exists():
        ...
        try:
            ...
            flash(f'Pre-built agent not found for {platform}. Available files: {", ".join([f.name for f in available_files]) if available_files else "none"}. Please build the agents first.', 'error')
        except Exception as e:
